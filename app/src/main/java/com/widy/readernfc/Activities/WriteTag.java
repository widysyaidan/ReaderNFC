...
public class WriteTag extends BasicActivity{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            mWriteManufBlock.setChecked(savedInstanceState.getBoolean("write_manuf_block", false));
    ...
    private int checkAccessConditions(String sectorTrailer, boolean showToasts) {
        ...
        if (acMatrix == null) {
            if (showToasts) {
                Toast.makeText(this, R.string.info_ac_format_error, Toast.LENGTH_LONG).show();
            }
            return 1;
        }
    ...
    public void onWriteDump(View view) {
        if (mEnableStaticAC.isChecked()) {
            ...
            if (!ac.matches("[0-9A-Fa-f]+")) {
                Toast.makeText(this, R.string.info_ac_not_hex, Toast.LENGTH_LONG).show();
                return;
            }
            if (ac.length() != 6) {
        ...
        if (mWriteDumpFromEditor) {
            ...
            intent.putExtra(FileChooser.EXTRA_BUTTON_TEXT, getString(R.string.action_write_full_dump));
    ...
    private void checkDumpAndShowSectorChooserDialog(final String[] dump) {
        ...
        Integer[] sectors = mDumpWithPos.keySet().toArray(new Integer[0]);
        Arrays.sort(sectors);
        ...
        final AlertDialog dialog = new AlertDialog.Builder(this)
                ...
                .setPositiveButton(R.string.action_ok, (dialog12, which) -> {})
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                    ...
                    if (mDumpWithPos.size() == 0) {
                        Toast.makeText(context, R.string.info_nothing_to_write, Toast.LENGTH_LONG).show();
                        return;
                    }
    ...
    private void createKeyMapForDump() {
        ...
        intent.putExtra(KeyMapCreator.EXTRA_KEYS_DIR, Common.getFile(Common.KEYS_DIR).getAbsolutePath());
        intent.putExtra(KeyMapCreator.EXTRA_SECTOR_CHOOSER, false);
        intent.putExtra(KeyMapCreator.EXTRA_SECTOR_CHOOSER_FROM, (int) Collections.min(mDumpWithPos.keySet()));
        intent.putExtra(KeyMapCreator.EXTRA_SECTOR_CHOOSER_TO, (int) Collections.max(mDumpWithPos.keySet()));
        intent.putExtra(KeyMapCreator.EXTRA_BUTTON_TEXT, getString(R.string.action_create_key_map_and_write_dump));
        startActivityForResult(intent, CKM_WRITE_DUMP);
    }
    private void checkDumpAgainstTag() {
        MCReader reader = Common.checkForTagAndCreateReader(this);
        if (reader == null) {
            Toast.makeText(this, R.string.info_tag_lost_check_dump, Toast.LENGTH_LONG).show();
            return;
        }
        ...
        HashMap<Integer, HashMap<Integer, Integer>> writeOnPos = reader.isWritableOnPositions(dataPos, keyMap);
        reader.close();
        ...
        for (int sector : mDumpWithPos.keySet()) {
            if (keyMap.indexOfKey(sector) < 0) {
                addToList(list, getString(R.string.text_sector) + ": " + sector, getString(R.string.text_keys_not_known));
            } else {
                sectors.add(sector);
            }
        }
        for (int sector : sectors) {
            if (writeOnPos.get(sector) == null) {
                addToList(list, getString(R.string.text_sector) + ": " + sector, getString(R.string.text_invalid_ac_or_sector_dead));
                continue;
            }
            ...
            for (int block : blocks) {
                ...
                if (!mWriteManufBlock.isChecked() && sector == 0 && block == 0) {
                    continue;
                } else if (mWriteManufBlock.isChecked() && sector == 0 && block == 0) {
                ...
                if ((sector < 31 && block == 3) || sector >= 31 && block == 15) {
                    String sectorTrailer = Common.bytes2Hex(mDumpWithPos.get(sector).get(block));
        ...
        if (list.size() != 0) {
            ...
            ListAdapter adapter = new SimpleAdapter(this, list, android.R.layout.two_line_list_item, from, to);
            listView.setAdapter(adapter);
            new AlertDialog.Builder(this)
                    .setTitle(R.string.dialog_write_issues_title)
                    .setIcon(android.R.drawable.ic_dialog_alert)
                    .setView(ll)
                    .setPositiveButton(R.string.action_skip_blocks, (dialog, which) -> {
                                writeDump(writeOnPosSafe, keyMap);
                            })
                    .setNegativeButton(R.string.action_cancel_all, (dialog, which) -> {
                            })
                    .show();
        } else {
            writeDump(writeOnPosSafe, keyMap);
    ...
    private void writeDump(final HashMap<Integer, HashMap<Integer, Integer>> writeOnPos, final SparseArray<byte[][]> keyMap) {
        ...
        new Thread(() -> {
            ...
            reader.close();
            warning.cancel();
            handler.post(() -> Toast.makeText(a, R.string.info_write_successful, Toast.LENGTH_LONG).show());
            a.finish();
        }).start();
    }
    ...
    public void onFactoryFormat(View view) {
        ...
        intent.putExtra(KeyMapCreator.EXTRA_KEYS_DIR, Common.getFile(Common.KEYS_DIR).getAbsolutePath());
        intent.putExtra(KeyMapCreator.EXTRA_SECTOR_CHOOSER, false);
        intent.putExtra(KeyMapCreator.EXTRA_BUTTON_TEXT, getString(R.string.action_create_key_map_and_factory_format));
    ...
    private void writeValueBlock() {
        ...
        if (keys[1] != null) {
            result = reader.writeValueBlock(sector, block, value, mIncreaseVB.isChecked(), keys[1], true);
        }
        if (result == -1 && keys[0] != null) {
            result = reader.writeValueBlock(sector, block, value, mIncreaseVB.isChecked(), keys[0], false);
        }
        reader.close();
        switch (result) {
            case 2:
